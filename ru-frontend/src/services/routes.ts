...
const desativarFuncionarioByCpf = (cpf: string) => {
  return api.post(
    `/funcionario/${cpf}/desativar/?data_saida=${new Date().toISOString().slice(0, 10)}`,
  )
}
...
const getAlmocos = (week: any) => {
  return api.get(`/compra/?refeicao=almoço&data_inicio=${week.ini}&data_fim=${week.fim}`)
}

const getJantas = (week: any) => {
  return api.get(`/compra/?refeicao=jantar&data_inicio=${week.ini}&data_fim=${week.fim}`, {})
}
