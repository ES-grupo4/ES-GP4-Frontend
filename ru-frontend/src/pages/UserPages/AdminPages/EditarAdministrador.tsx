...
export default function EditarAdministrador({
		...
		if (id != null) {
			...
			if (administradores.length !== 0) {
				setAdministradorData({
					cpf: administradores[0].cpf,
					nome: administradores[0].nome,
					email: administradores[0].email,
				});
				setFound(true);
			}
		}
	...
	const excluirAdministrador = async () => {
		...
		const confirmation = confirmationRequired
			? confirm(
					"O usuário a ser desativado é o mesmo que está logado. Prosseguir?",
				)
			: false;
		if (
			(!confirmationRequired || (confirmationRequired && confirmation)) &&
			id != null
		) {
			try {
				const response = await routes.removeFuncionarioByCpf(
					administradorData.cpf,
				);
			...
			{found ? (
				<div className="bg-white mx-auto my-25 p-5 w-7/10 h-130 rounded-lg">
					<p className="text-2xl">
						<span className="font-bold">CPF:</span> {administradorData.cpf}
					</p>
					<p className="text-2xl">
					...
					<div className="mt-20 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
						...
						<input
							type="text"
							id="name"
							value={administradorData.nome}
							onChange={onChangeName}
							className="bg-gray-200 border border-gray-300 text-gray-900 text-m rounded-lg block w-3/4 p-2.5"
					...
					<div className="mt-5 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
						...
						<input
							...
							className="bg-gray-200 disabled:bg-gray-500 border border-gray-300 text-gray-900 text-m rounded-lg block w-3/4 p-2.5"
					...
					<div className="mt-20 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
						<button
							...
							className="cursor-pointer bg-green-700 hover:bg-green-800 focus:ring-4 rounded-lg text-m text-white font-bold  px-5 py-2.5 me-2 mb-2"
						>
							Salvar Alterações
						</button>
						<RemoveButton
							onClickFunction={excluirAdministrador}
							nomeEntidade={"administrador"}
						/>
					</div>
				</div>
			) : (
				<span className="text-2xl m-auto text-red-500">
					Não foi possível encontrar administrador com ID {id}
				</span>
			)}
		</div>
	);
