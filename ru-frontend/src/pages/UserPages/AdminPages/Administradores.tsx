  ...
  useEffect(() => {
    const getAdminData = async () => {
      ...
      const data = response.data.items.filter(
        (func: { [x: string]: string }) => func.tipo === 'admin',
      )
  ...
  return (
    <div className='p-4 sm:ml-64'>
      <div className='group flex'>
        ...
        <Link
          to={UrlRouter.usuario.administracao.administradores.adicionar}
          className='bg-green-700 hover:bg-green-950 rounded-xl h-15 w-15 mx-4 my-2 cursor-pointer'
          type='button'
        >
          <svg
      ...
      <EntityTable
        tableData={data}
        columns={['id', 'nome', 'cpf', 'email', 'data_entrada']}
        url={UrlRouter.usuario.administracao.administradores.editar.split(':')[0]}
        hasChart={false}
        chartUrl=''
        page={page}
        total_pages={pageQtd}
