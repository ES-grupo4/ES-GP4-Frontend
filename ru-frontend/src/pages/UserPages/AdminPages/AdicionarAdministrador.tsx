...
function AdicionarAdministrador(): ReactElement {
	...
	const addAdminJson = async (jsonData: RowData[]) => {
		...
		if (errors.length > 0) {
			alert(
				`Alguns administradores não foram cadastrados:\n${errors.join("\n")}`,
			);
		} else {
	...
	const verifyAdmin = (adminData: Record<string, unknown>) => {
		if (
			adminData.cpf === undefined ||
			adminData.nome === undefined ||
			adminData.email === undefined ||
			adminData.senha === undefined
		) {
			return "Campos obrigatórios faltando!";
		} else if (
			typeof adminData.cpf === "string" &&
			!validarCPF(adminData.cpf)
		) {
			return "CPF inválido";
		}
		return "OK";
